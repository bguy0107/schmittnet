...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const note = await ticketService.addPublicNote(token, ticketId, body);
    return NextResponse.json(note, { status: 201 });
  } catch (error) {
    if (error instanceof AppError) {
      return NextResponse.json(toApiError(error), { status: error.statusCode });
    }
    logger.error("POST /api/tickets/submit/[token]/tickets/[ticketId]/notes unexpected error", { error: String(error) });
