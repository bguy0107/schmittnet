...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const result = await ticketService.cancelPublicTicket(token, ticketId, body);
    return NextResponse.json(result);
  } catch (error) {
    if (error instanceof AppError) {
      return NextResponse.json(toApiError(error), { status: error.statusCode });
    }
    logger.error("PATCH /api/tickets/submit/[token]/tickets/[ticketId]/cancel unexpected error", { error: String(error) });
