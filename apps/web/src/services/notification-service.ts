...
export type NotificationJobData =
  ...
  | { type: "TICKET_IN_PROGRESS"; ticketId: string; locationId: string; category: Category }
...
export const notificationService = {
  ...
  async enqueueTicketInProgress(ticketId: string, locationId: string, category: Category) {
    await getQueue()
      .add("ticket-in-progress", { type: "TICKET_IN_PROGRESS", ticketId, locationId, category })
      .catch((err: unknown) =>
        logger.error("Failed to enqueue TICKET_IN_PROGRESS notification", {
          ticket_id: ticketId,
          error: String(err),
        }),
      );
